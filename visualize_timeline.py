...
def load_photo_locations_from_csv(file_path: str) -> pd.DataFrame | None:
    """Loads photo location data from CSV, parses timestamps (UTC), and sorts by timestamp."""
...
def filter_locations_dataframe(df: pd.DataFrame, start_date: datetime, end_date: datetime) -> pd.DataFrame:
    """Filters DataFrame by date range (inclusive). Dates should be timezone-aware (UTC recommended)."""
    if df['timestamp'].dt.tz is None:
        raise ValueError("DataFrame timestamps must be timezone-aware for filtering.")
    if start_date.tzinfo is None or end_date.tzinfo is None:
        raise ValueError("Start and end dates must be timezone-aware for filtering.")
...
def create_interactive_map(
    ...
    try:
        if len(gdf_wgs84) > 1:
            center_lat, center_lon = gdf_wgs84.union_all().centroid.y, gdf_wgs84.union_all().centroid.x
        elif len(gdf_wgs84) == 1:
            center_lat, center_lon = gdf_wgs84.iloc[0].geometry.y, gdf_wgs84.iloc[0].geometry.x
        else:
            center_lat, center_lon = 0, 0
        start_location = [center_lat, center_lon]
    except Exception as e:
        print(f"Warning: Could not calculate center, using default. Error: {e}")
        start_location = [0, 0]
    ...
    if len(gdf_wgs84) >= 2:
        ...
        folium.PolyLine(locations=locations, color=line_color, weight=line_weight, opacity=1.0, tooltip=f"Photo Path ({len(locations)} points)").add_to(m)
...
if __name__ == "__main__":
    ...
    start_date_str = None
    end_date_str = None
    output_map_filename = 'interactive_photo_path_map.html'
    ...
    if csv_file_path == './local_photo_locations.csv' and not os.path.exists(csv_file_path):
        ...
        print(f"ERROR: The specified CSV file path does not exist: {csv_file_path}")
        sys.exit(1)
    ...
    if start_date_str and end_date_str:
        ...
        try:
            ...
            start_datetime_utc = start_dt.replace(tzinfo=timezone.utc)
            end_datetime_utc = end_dt.replace(hour=23, minute=59, second=59, microsecond=999999, tzinfo=timezone.utc)
            perform_filtering = True
            print("Date range parsed successfully.")
        except ValueError:
            print(f"Error: Invalid date format. Disabling date filtering.")
            perform_filtering = False
    else:
        print("No date range specified. Processing all data.")

    # --- Load & Filter Data ---
    try:
        all_photos_df = load_photo_locations_from_csv(csv_file_path)
        if all_photos_df is None or all_photos_df.empty:
            print("No valid photo location data loaded. Exiting.")
            sys.exit(0)
        ...
        if photos_to_plot_df is None or photos_to_plot_df.empty:
            print("No photos found for the specified criteria. No map generated.")
        else:
            # --- Create GeoDataFrame ---
            ...
            try:
                ...
                print(f"Error creating GeoDataFrame or Map: {e_map}")
                import traceback
                traceback.print_exc()
